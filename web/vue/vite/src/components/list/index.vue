<template>
              ...
              <el-table-column :prop="key" :label="item.title">
                <template #default="scope">
                  <el-link type="primary" @click="showList({ [key]: scope.row[key] }, item.oneToMany, item.config)">
                    <app-form-input v-model="scope.row" mode="details" :schema="item" :prop="key" />
                  </el-link>
                </template>
              </el-table-column>
            </template>
                ...
                <el-table-column :prop="key + '.' + key2">
                  ...
                  <template #default="scope">
                    <template v-if="scope.row[key]">
                      <app-form-input v-model="scope.row[key]" mode="details" :schema="item2" :prop="key2" />
                    </template>
                  </template>
                </el-table-column>
              </template>
  ...
  <el-drawer v-model="subDrawer" :close-on-click-modal="false" destroy-on-close size="50%">
    <el-scrollbar>
          ...
          <template v-else-if="editFormMode === 'filter'">
            <el-form :model="filterList" inline class="filter">
              <el-row v-for="(item, index) in filterList.filter((o) => !o.hidden)">
                <el-col :span="6">
                  <el-select v-model="item.column" clearable :disabled="item.readOnly" :placeholder="$t('字段')">
                ...
                <el-col v-if="item.column" :span="6">
                  <el-select v-model="item.action" clearable :disabled="item.readOnly" :placeholder="$t('操作符')">
                    <el-option
...
<script setup>
  ...
  import { format, importFunction, listToTree, schemaToModel } from '../../utils/index.js';
  import request, { getUrl } from '../../utils/request.js';

  const props = defineProps(['modelValue', 'config', 'querySchema', 'query', 'buttons']);
